...
from app.repositories.analysis_repository import (  # noqa: E402
    analysis_job_to_dict,
    create_analysis_job,
    get_latest_analysis_job_by_session,
    update_analysis_job,
)
...
from app.repositories.verification_repository import (  # noqa: E402
    create_verification,
    get_latest_verification_by_session,
...
def main() -> None:
    ...
    with SessionLocal() as db:
        latest_job = get_latest_analysis_job_by_session(db, session_id)
        latest_report = get_latest_report_by_session(db, session_id)
        latest_verification = get_latest_verification_by_session(db, session_id)

        payload = {
            "sessionId": session_id,
            "analysis": analysis_job_to_dict(latest_job) if latest_job else None,
            "report": report_to_dict(db, latest_report) if latest_report else None,
            "verification": verification_to_dict(latest_verification) if latest_verification else None,
        }
